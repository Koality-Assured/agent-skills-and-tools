...
Validating one skill (`skill-dry-run`), one structure check (`wiki-structure`), a standalone cost-layer measurement (`cost-layer-dry-run`), or a single code artifact (`code-review-report`).
...
- `python scripts/ai-tooling/validate_skill.py --all`, `validate_agent.py --all`, and `python scripts/docs/validate_wiki_structure.py`
