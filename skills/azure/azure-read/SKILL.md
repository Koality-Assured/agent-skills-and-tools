...
name: azure-read
description: >-
  Read-only Azure inventory/status via OAuth or named profiles; summarize into
  results/. Use when the human asks to inspect Azure without changes. Do not use
  for writes (azure-write) or log deep-dives (azure-logs).
owner_agent: cloud-operator
rank: medium
isolation: mutate
contracts:
  inputs:
    - Azure profile or OAuth and the inventory or status question
  outputs:
    - Read-only inventory summary path and high-level findings under results/ (not full API dumps)
---
...
Read-only Azure inspection via OAuth / named profiles; summarize under `results/`.

## When not to use

Mutating Azure (`azure-write` — requires human-turn auth). Log analysis (`azure-logs`). Committing credentials. Destructive changes without write skill + auth.
...
- Azure CLI/SDK via OAuth or named profiles
...
2. Perform read-only Azure queries needed for the ask.
...
Never commit creds/tokens. A2A MUST NOT destructive-delegate. For changes, require `azure-write` + human-turn authorization.
